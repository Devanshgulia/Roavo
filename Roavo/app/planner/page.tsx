...
const PREDEFINED_INTERESTS = [
  'Beaches', 'Adventure', 'Food', 'Culture', 'Shopping',
  'Photography', 'Nightlife', 'Nature', 'Relaxation',
  'Hiking', 'Museums', 'Local Experiences'
];
...
function PlannerPageClient() {
  ...
  useEffect(() => {
    const detectUserLocation = async () => {
      ...
      try {
        ...
        if (!geoResponse.ok) {
          console.warn('Rate limited or blocked from location detection, skipping autodetect.');
          return;
        }
  ...
  const [locationSuggestions, setLocationSuggestions] = useState<Array<{ id: string, place_name: string }>>([]);
  ...
  useEffect(() => {
    ...
    const currentName = formData.destinations[activeLocationIndex]?.name;
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        const response = await fetch(`/api/location-search?q=${encodeURIComponent(currentName)}`);
        if (response.ok) {
  ...
  }, [activeLocationIndex, activeLocationIndex !== null ? formData.destinations[activeLocationIndex]?.name : '']);

  // Auto-calculate return date based on total duration
  useEffect(() => {
    if (formData.startDate && formData.destinations && formData.destinations.length > 0) {
      const totalDays = formData.destinations.reduce((sum, dest) => sum + (Number(dest.duration) || 0), 0);
      if (totalDays >= 1) {
        ...
        if (!isNaN(start.getTime())) {
          ...
          if (formData.endDate !== formattedReturnDate) {
            setFormData(prev => ({ ...prev, endDate: formattedReturnDate }));
          }
        }
      }
    }
  }, [formData.startDate, formData.destinations]);
                  ...
                  return (
                    <div key={num} className="flex flex-col items-center relative">
                      ...
                      <div className="text-center">
                        <div className={`text-sm font-medium transition-colors duration-300 ${isActive ? 'text-white' : 'text-white/60'
                          }`}>
                          {num === 1 && 'Destinations'}
                          {num === 2 && 'Details'}
                          {num === 3 && 'Preferences'}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {(showAllSuggestions ? suggestedLocations : suggestedLocations.slice(0, 6)).map((location, index) => {
                      ...
                      return (
                        ...
                        >
                          <div className={`card-elevated p-6 h-full transition-all duration-300 ${isSelected
                            ? 'ring-4 ring-teal-400 bg-gradient-to-br from-teal-50 to-cyan-50 border-teal-200'
                            : 'hover:shadow-lg border-gray-200'
                            }`}>

                            <div className={`absolute top-4 right-4 w-6 h-6 rounded-full transition-all duration-300 ${isSelected
                            ...
                            <div className="absolute top-4 left-4">
                              <span className={`inline-block px-2 py-1 rounded-full text-xs font-medium ${isSelected
                            ...
                            <div className="mt-8 mb-4">
                              <h5 className={`text-xl font-display mb-1 ${isSelected ? 'text-teal-900' : 'text-gray-900'
                                }`}>
                                {location.name}
                              </h5>
                              <p className={`text-sm font-medium ${isSelected ? 'text-teal-700' : 'text-gray-600'
                                }`}>
                                {location.country}
                              </p>
                            </div>

                            <p className={`text-sm leading-relaxed ${isSelected ? 'text-teal-800' : 'text-gray-700'
                              }`}>
                              {location.reason}
                            </p>
                                ...
                                <div>
                                  ...
                                  <div className="flex flex-wrap gap-2">
                                    {detectedInterests.map((interest, index) => (
                                      <span
                                        key={index}
                                        className={`inline-flex items-center px-3 py-1 rounded-full text-sm ${selectedInterests.includes(interest)
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                          <div className="md:col-span-2 relative">
                            ...
                            <input
                              ...
                              onBlur={() => setTimeout(() => { if (activeLocationIndex === index) setActiveLocationIndex(null) }, 200)}
                              className="form-input w-full"
                              placeholder="e.g., Paris, Bali, Morocco"
                            />
                            
                            {activeLocationIndex === index && (locationSuggestions.length > 0 || isSearchingLocation) && (
                                  ...
                                  <ul className="max-h-60 overflow-y-auto">
                                    {locationSuggestions.map((suggestion) => (
                                      <li
                                        ...
                                        onMouseDown={(e) => {
                                          e.preventDefault();
                                          updateDestination(index, 'name', suggestion.place_name);
                                          setActiveLocationIndex(null);
                                        }}
                                        className="px-4 py-3 text-sm text-gray-700 hover:bg-teal-50 cursor-pointer border-b border-gray-100 last:border-0 transition-colors"
                                      >
                      ...
                      <div className="space-y-4">
                        ...
                        <div>
                          <div className="flex justify-between items-end mb-3">
                            ...
                            <div className="text-xs font-semibold text-teal-700 bg-teal-100/50 px-2 py-1.5 rounded-lg border border-teal-200 inline-flex items-center">
                              Total Trip Duration: {formData.destinations.reduce((sum, dest) => sum + (Number(dest.duration) || 0), 0)} days
                            </div>
                          </div>
                          <input
                            type="date"
                            ...
                            readOnly
                            className="w-full px-6 py-4 bg-teal-50/50 rounded-xl border border-teal-200 text-teal-900 cursor-not-allowed opacity-90 shadow-inner"
                          />
                          <p className="mt-2 text-xs text-teal-600 font-medium">
                            Return date is automatically calculated based on your trip duration
                          </p>
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <div className="space-y-3">
                          {TRAVEL_STYLES.map(style => (
                            <div
                              key={style.value}
                              className={`p-4 border rounded-xl cursor-pointer transition-all ${formData.travelStyle === style.value
                    ...
                    <div className="space-y-6">
                      ...
                      <div className="space-y-4">
                        {BUDGET_OPTIONS.map(option => (
                          <div
                            key={option.value}
                            className={`p-6 border rounded-xl cursor-pointer transition-all ${formData.budget === option.value
                      ...
                      <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
                        <div className="flex items-center justify-between mb-4">
                          ...
                          <button
                            ...
                            className={`w-12 h-6 rounded-full transition-all duration-300 ${wantsHotelRecommendations ? 'bg-green-500' : 'bg-gray-400'
                              }`}
                          >
                            <div className={`w-5 h-5 bg-white rounded-full transition-all duration-300 ${wantsHotelRecommendations ? 'translate-x-6' : 'translate-x-0.5'
                              }`} />
                          </button>
                        </div>
                      </div>
                      ...
                      <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
                        <div className="flex items-center justify-between mb-4">
                          ...
                          <button
                            ...
                            className={`w-12 h-6 rounded-full transition-all duration-300 ${wantsFlightBooking ? 'bg-green-500' : 'bg-gray-400'
                              }`}
                          >
                            <div className={`w-5 h-5 bg-white rounded-full transition-all duration-300 ${wantsFlightBooking ? 'translate-x-6' : 'translate-x-0.5'
                              }`} />
                          </button>
                        </div>
                      </div>
                  ...
                  <div>
                      ...
                      <div className="mb-6">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                          {aiSuggestions.map((suggestion, index) => (
                            <div
                              key={index}
                              className={`p-3 border rounded-lg cursor-pointer transition-all text-sm ${formData.interests.includes(suggestion.name)
                                ? 'border-teal-400 bg-teal-50 ring-2 ring-teal-400/50'
                    ...
                    <div className="bg-gray-50 p-6 rounded-xl border border-gray-200">
                      <div className="mb-6">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {PREDEFINED_INTERESTS.map((interest, index) => {
                            ...
                            return (
                              <button
                                ...
                                className={`px-4 py-2 rounded-full text-sm font-medium transition-all duration-200 ${isSelected
                                  ...
                                  : 'bg-white text-gray-700 border border-gray-300 hover:border-teal-400 hover:text-teal-600 shadow-sm hover:shadow-md'
                                  }`}
                              >
                                {interest}
                              </button>
                      ...
                      <div>
                        ...
                        <div className="space-y-3">
                          {ACCOMMODATION_TYPES.map(type => (
                            <div
                              key={type.value}
                              className={`p-4 border rounded-xl cursor-pointer transition-all ${formData.accommodation === type.value
                    ...
                    <div>
                      ...
                      <div className="space-y-3">
                        {TRANSPORT_OPTIONS.map(option => (
                          <div
                            key={option.value}
                            className={`p-4 border rounded-xl cursor-pointer transition-all ${formData.transportPreference === option.value
                              ? 'border-green-400 bg-green-50 ring-2 ring-green-400/50'
                              : 'border-gray-200 bg-white hover:border-gray-300'
                              }`}
