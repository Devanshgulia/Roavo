  ...
  const downloadPDFItinerary = async () => {
    ...
    try {
      ...
      if (data) {
        ...
        pdf.setFontSize(11);
        pdf.text(`Total Estimated Cost: ${data?.budget?.total || "Not available"}`, 10, yPosition);
        yPosition += 5;
        pdf.text(`Daily Average: ${data?.budget?.dailyAverage || "N/A"}`, 10, yPosition);
        yPosition += 10;
        ...
        pdf.setFontSize(10);
        pdf.text(`• Accommodation: ${data.budget?.breakdown?.accommodation || 'N/A'}`, 15, yPosition);
        yPosition += 5;
        pdf.text(`• Food: ${data.budget?.breakdown?.food || 'N/A'}`, 15, yPosition);
        yPosition += 5;
        pdf.text(`• Activities: ${data.budget?.breakdown?.activities || 'N/A'}`, 15, yPosition);
        yPosition += 5;
        pdf.text(`• Transport: ${data.budget?.breakdown?.transport || 'N/A'}`, 15, yPosition);
        yPosition += 5;
        pdf.text(`• Extras: ${data.budget?.breakdown?.extras || 'N/A'}`, 15, yPosition);
  ...
  return (
    <div className="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-200">
      ...
      <div className="p-6">
        ...
        {activeTab === 'transport' && wantsFlightBooking && data?.transport && (
          <div className="space-y-6">
            ...
            {data.transport.gettingThere?.flights && data.transport.gettingThere.flights.length > 0 && (
            ...
            {data.transport.localTransport && data.transport.localTransport.length > 0 && (
        ...
        {activeTab === 'accommodation' && wantsHotelRecommendations && data?.accommodation?.recommendations && (
          <div className="space-y-6">
            <h2 className="text-2xl font-bold text-gray-900 flex items-center">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="bg-green-50 border border-green-200 rounded-xl p-6">
                ...
                <div className="text-center mb-6">
                  <div className="text-4xl font-bold text-green-600 mb-2">{data?.budget?.total || "Not available"}</div>
                  <div className="text-green-700">Daily Average: {data?.budget?.dailyAverage || "N/A"}</div>
                </div>
                
                <div className="space-y-3">
                  {data?.budget?.breakdown && Object.entries(data.budget.breakdown).map(([category, amount]) => (
